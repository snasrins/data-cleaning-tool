...
def main():
    ...
    excel_files = get_excel_files(BASE_DIR)
    out_folder = BASE_DIR / "Cleaned"
    ...
    for file in excel_files:
        ...
        # Save cleaned Excel for this file
        cleaned_name = f"Cleaned_{file.stem}.xlsx"
        out_excel = out_folder / cleaned_name
        save_cleaned_excel(df, out_excel)
        print(f"  Saved cleaned file: {out_excel}")
    ...
    messagebox.showinfo(
        "Done",
        f"All files cleaned!\n\nSaved to: {out_folder}\n\n" + '\n'.join([f"• Cleaned_{f.stem}.xlsx" for f in excel_files]),
    )
    root.destroy()
